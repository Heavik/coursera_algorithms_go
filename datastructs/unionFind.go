...
func InitUnionFind(nums int) UnionFind {
	ds := disjointSet{elements: make([]int, nums+1), size: make([]int, nums+1)}
	for i := 1; i <= nums; i++ {
		ds.elements[i] = i
		ds.size[i] = 1
	}
...
func UnionFindFromSlice(slice []int) UnionFind {
	ds := disjointSet{elements: make([]int, len(slice)), size: make([]int, len(slice))}
	for i := 0; i < len(slice); i++ {
		ds.elements[i] = slice[i]
		ds.elements[i] = 1
	}
	return &ds
}

func (s *disjointSet) Union(s1, s2 int) {
...
func (s *disjointSet) Find(val int) int {
...
func (s *disjointSet) Connected(v1, v2 int) bool {
